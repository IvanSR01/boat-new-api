...
class AuthController {
    async getCode(req, res) {
        try {
            ...
            const { error, code } = await callApiService.flashCall(userService.setPhone(phone));
            if (error.isError)
    ...
    async sellerRegistration(req, res) {
        try {
            const { email, phone, name, surname, password, confirm, personalUrl, paymentInfo, } = req.body;
            const { user: userByMail } = await userService.checkUser(email);
            if (userByMail)
                return res.status(400).json({
                    message: 'Этот почта занята',
